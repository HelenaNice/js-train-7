...
function createSymbolWithDescription(property) {
  let symbol = Symbol(property); // Створюємо символ з описом property
...
function createSymbolProperty(property) {
  let sym = Symbol(); // Створюємо змінну sym символ без опису
  let obj = {}; // Створюємо пустий об'єкт
  obj[sym] = property; // Додаємо властивість до об'єкту з ключем, яким є наш символ, а значенням - аргумент property
...
let symbol1 = Symbol(value); // Створюємо перший символ з описом value
 let symbol2 = Symbol(value); // Створюємо другий символ з описом value
  let compare = symbol1 === symbol2; // Порівнюємо два символи
  return compare; // Повертаємо результат порівняння}
}
